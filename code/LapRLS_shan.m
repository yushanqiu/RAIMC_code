function [score] = LapRLS_shan(W1,W2,y, lambda,p_nearest_neighbor,type)
...
%     temp=H.*(S1'*y'*(S2*W));
%     size(temp)
%     temp2=(S1'*(S1*H)*(W'*(S2')*(S2*W))+H);
%     size(temp2)
    H=H.*(S1'*y'*(S2*W))./(S1'*(S1*H)*(W'*(S2')*(S2*W))+H);
    W=W.*(S2'*y*(S1*H))./(S2'*(S2*W)*(H'*(S1')*(S1*H))+W);
